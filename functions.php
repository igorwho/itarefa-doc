<?php
    ...
    function get_topics () {
        ...
        foreach($FILES as $temp) {
            $handle = @fopen($FOLDER . '/' . $temp, "r");
            if ($handle) {
                while (($line = fgets($handle, 4096)) !== false) {
                    if (strpos($line, '# ') !== False) {
                        $r[] = [trim(explode('# ', $line)[1]), $temp];
                        break;
                    }
                }
                fclose($handle);
    ...
    $ICON_WARNING = '<svg width="1.0625em" height="1em" viewBox="0 0 17 16" class="bi bi-exclamation-triangle" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
